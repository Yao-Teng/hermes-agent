...
class CurlCffiImpersonateTransport(httpx.AsyncBaseTransport):
    ...
    Some networks (GFW-style TLS fingerprint filtering) drop connections using
    Python/OpenSSL's default TLS fingerprint while letting real browsers
    through. This transport routes every request through ``curl_cffi`` with an
    ``impersonate`` profile (chrome/safari/firefox) so the TLS handshake
    matches a real browser. Requires the optional ``curl-cffi`` package; if it
    is missing the constructor raises ``ImportError`` and callers fall back to
    the default stack.
    """
    ...
    def _ensure_session(self) -> None:
        # PTB shuts down the HTTPXRequest (and with it our AsyncSession) after a
        # network error, then resumes polling with a fresh client that reuses
        # this same transport. curl_cffi raises SessionClosed once close() has
        # run, so recreate the session lazily instead of leaking the error into
    ...
    async def aclose(self) -> None:
        try:
            ...
            # curl_cffi 0.16's AsyncSession.close() can raise on Windows asyncio
            # teardown (ctype NoneType) after the loop is closing; never let
            # transport cleanup crash the gateway shutdown.
            pass
