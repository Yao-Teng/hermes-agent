...
from plugins.platforms.telegram.telegram_network import (
    SEED_FALLBACK_IPS, CurlCffiImpersonateTransport, TelegramFallbackTransport, discover_fallback_ips,
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _build_ptb_requests(self) -> tuple:
        ...
        # Optional TLS-fingerprint impersonation (HERMES_TELEGRAM_IMPERSONATE=chrome|safari|firefox).
        # For networks that drop Python/OpenSSL TLS fingerprints while allowing real browsers;
        # requires the optional curl-cffi package, falls back to the default stack when absent.
        ...
        if impersonate and not proxy_url:
            try:
                ...
                request, get_updates_request = _pair(
                    {"transport": CurlCffiImpersonateTransport(impersonate)},
                    {"transport": CurlCffiImpersonateTransport(impersonate)})
                return request, self._instrument_polling_request(get_updates_request)
            except ImportError:
                logger.warning(
                    "[%s] HERMES_TELEGRAM_IMPERSONATE set but curl-cffi is not installed; "
                    "falling back to default transport", self.name)
